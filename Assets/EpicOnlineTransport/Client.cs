...
namespace EpicTransport {
    public class Client : Common {
        ...
        public static Client CreateClient(EosTransport transport, string host) {
            ...
            c.OnConnected += () => { if (transport != null) transport.OnClientConnected.Invoke(); };
            c.OnDisconnected += () => { if (transport != null) transport.OnClientDisconnected.Invoke(); };
            c.OnReceivedData += (data, channel) => { if (transport != null) transport.OnClientDataReceived.Invoke(new ArraySegment<byte>(data), channel); };
        ...
        public async void Connect(string host) {
            ...
            try {
                ...
                if (await Task.WhenAny(connectedCompleteTask, Task.Delay(ConnectionTimeout/*, cancelToken.Token*/)) != connectedCompleteTask) {
                    Debug.LogError($"[Client] Connection to {host} timed out.");
                    OnConnected -= SetConnectedComplete;
                ...
                Debug.LogError($"[Client] Connection string was not in the right format. Did you enter a ProductId?");
                Error = true;
                OnConnectionFailed(hostProductId);
            } catch (Exception ex) {
        ...
        protected override void OnNewConnection(ref OnIncomingConnectionRequestInfo result) {
            ...
            if (hostProductId == result.RemoteUserId) {
                ...
                EOSSDKComponent.GetP2PInterface().AcceptConnection(ref acceptOptions);
            } else {
        ...
        protected override void OnReceiveInternalData(InternalMessages type, ProductUserId clientUserId, SocketId socketId) {
            ...
            switch (type) {
                ...
                case InternalMessages.DISCONNECT:
                    ...
                    break;
                default:
                    Debug.Log("[Client] Received unknown message type");
                    break;
            }
        }
