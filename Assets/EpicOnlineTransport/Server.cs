...
namespace EpicTransport {
    public class Server : Common {
        ...
        private event Action<int, Exception> OnReceivedError;  // kept as Exception internally
            ...
            switch (type) {
                case InternalMessages.CONNECT:
                    if (epicToMirrorIds.Count >= maxConnections) {
                        Debug.LogError("[Server] Reached max connections");
                    ...
                    Debug.Log($"[Server] Client with Product User ID {clientUserIdString} connected. Assigning connection id {connectionId}");
                    break;
                case InternalMessages.DISCONNECT:
                    if (epicToMirrorIds.TryGetValue(clientUserId, out int connId)) {
                        ...
                        Debug.Log($"[Server] Client with Product User ID {clientUserId} disconnected.");
                    } else {
                        OnReceivedError.Invoke(-1, new Exception("ERROR Unknown Product User ID"));
                    }

                    break;
                Debug.Log("[Server] Received unknown message type");
                    break;
            }
        }
